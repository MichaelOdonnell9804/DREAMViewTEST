...
# === (1) Define the ASCII “face” pattern ===
_pattern = [
    "      ******************      ",
    "      ******************      ",
    "     ********************     ",
...
# The module‐to‐(x,y) coordinates dictionary you already provided:
_modules = {
    ...
    "P": [(9,9),  (10,9), (11,9), (12,9),
          ...
          (9,7),  (10,7), (11,7), (12,7),
          (9,6),  (10,6), (11,6), (12,6)],
    "S": [(13,3), (14,3), (15,3), (16,3),
          (13,2), (14,2), (15,2), (16,2),
          (13,1), (14,1), (15,1), (16,1),
          (13,0), (14,0), (15,0), (16,0)],
    "R": [(13,7), (14,7), (15,7), (16,7),
          (13,6), (14,6), (15,6), (16,6),
          (13,5), (14,5), (15,5), (16,5),
          (13,4), (14,4), (15,4), (16,4)],
...
# Mapping of FERS board numbers to pairs of module letters.
# Board4 corresponds to modules N and O and so on. This mirrors the
# configuration used in ``plot_board.py``.
BOARD_TO_MODULES = {
    4: ("N", "O"),
    3: ("P", "Q"),
    2: ("R", "S"),
...
# Path to geometry.json relative to this file
_JSON_PATH = Path(__file__).parent / "geometry.json"


class Geometry:
    """
    Loads (or builds) geometry.json. Provides:
      • self.width, self.height
      • self.cells: list of {'x','y','type'}
      • self.modules: { letter: [ {'x','y'}, … ] }
      • self.channel_map: { int(channel) → (x,y) }
      • Helpers: is_scint_channel(channel), get_xy(channel), etc.
    """

    def __init__(self):
        # If geometry.json does not exist (or is empty), build it
        ...
        self.cells = data["cells"]           # List[{'x','y','type'}]
        # 'modules' is {letter: [ {'x','y'}, … ]}
        self.modules = data["modules"]
        # 'channel_map' is { "0": {'x','y'}, "1": {...}, … }
        # Convert keys to int and values to tuples
        self.channel_map = {
            int(k): (v["x"], v["y"]) for k, v in data["channel_map"].items()
        }

        # Build a quick reverse from (x,y) → channel index
        self._pos_to_chan = {
            (coords["x"], coords["y"]): int(ch)
            for ch, coords in data["channel_map"].items()
        }

        # Precompute, for each module letter, the list of channel‐indices (in order)
        self.module_channel_indices = {}
        for letter, coords_list in self.modules.items():
            # coords_list is a list of dict{'x','y'} of length 16
            chans = []
        ...
        # Union of all scintillator channels from the modules
        self.scint_channels = {
            ch for channels in self.module_channel_indices.values() for ch in channels
        }

        # Build a simple module_layout used by the old interface
        self.module_layout = {
            name: {
                "base": min(chs),
                "rows": 4,
            ...
            for name, chs in self.module_channel_indices.items()
        }

        # Expose the precomputed channel maps from utils.channel_map
    ...
    def is_scint_channel(self, channel: int) -> bool:
        """Return ``True`` if ``channel`` corresponds to a scintillator."""
        if channel not in self.channel_map:
            raise KeyError(f"Channel {channel} not found in geometry.")
        # ``scint_channels`` covers all scintillator locations derived from
        # ``module_channel_indices`` during initialization.
        return channel in self.scint_channels

    def get_xy(self, channel: int) -> tuple[int, int]:
        """
        Returns (x,y) for a given global channel index.
        """
        if channel not in self.channel_map:
            raise KeyError(f"Channel {channel} not found in geometry.")
    ...
    def get_scint_indices(self) -> list[int]:
        """Return a sorted list of all scintillator channel indices."""
        return sorted(self.scint_channels)

    def get_grid(self) -> np.ndarray:
        """Return a 2D array of cell types (0=blank,1=star,2=hash)."""
    ...
    def get_channel_positions(self):
        """Return dictionaries of channel → (x,y) and channel types."""
    ...
    def _build_geometry_json(self):
        """
        Construct `geometry.json` by:
          1) parsing _pattern → grid, cells
          2) using _modules to place the module slots
          3) building channel_map by enumerating all “star” cells in row-major
        """
        ...
        # 2) Build channel_map from every “star” (t==1), sorted row-major by (y,x)
        ...
        # 3) Convert _modules into the right JSON shape
        modules_out = {
            name: [{"x": x, "y": y} for (x, y) in coords]
            for name, coords in _modules.items()
        }
    ...
    def draw_face(self, ax, show_grid: bool = False):
        """Draw the detector face onto ``ax``.
        ...
        show_grid : bool, optional
            If True, draw a 4x4 grid inside each module to visualise the
            individual channels.
        """
        height = self.height
        width = self.width
        ...
        for name, coords in self.modules.items():
            ...
            ax.add_patch(plt.Rectangle((minx, miny), w_box, h_box,
                                       edgecolor='red', fill=False, linewidth=2))
            cx = (min(xs) + max(xs)) / 2
            cy = (min(ys) + max(ys)) / 2
            ax.text(cx, cy, name[0].upper(),
    ...
    def visualize(self, save_as: str | Path | None = None):
        """Convenience helper that draws the face on a new figure."""
...
# If you run geometry.py directly, rebuild & visualize:
